...
function buildWhatsAppLink() {
  ...
  // ✅ تنظيف عام للرابط/الرقم (بدون اعتماد على رقم ثابت)
  const raw = String(profile.links.whatsapp || "")
    .replace("https://wa.me/", "")
    .replace("wa.me/", "")
    .replace(/\D/g, "");
  ...
  const text = encodeURIComponent(
    "Hi Ellin, I found your portfolio and would love to connect."
  );

  // إذا ما في رقم مضبوط، رجّعي رابط واتساب عام (احتياط)
...
export default function Contact() {
  ...
  const emailTo = useMemo(() => profile.links.email.replace("mailto:", ""), []);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      // ✅ إرسال على الباك (Vercel Function)
